...
enum Fixups {
  ...
  fixup_riscv_lo12_s,
  // 12-bit fixup corresponding to %base_idx_lo(foo) for instructions like addi
  fixup_riscv_base_idx_lo12_i,
  // 12-bit fixup corresponding to %base_idx_lo(foo) for the S-type store
  // instructions
  fixup_riscv_base_idx_lo12_s,
  // Fixup corresponding to %base_idx_add(foo) for PseudoAddBaseIdx/
  // PseudoAddUWBaseIdx, used as a linker hint
  fixup_riscv_base_idx_add,
